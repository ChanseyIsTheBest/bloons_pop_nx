...
/* Movie audio ring.
 *
 * INHERITED SHAPE, CHANGED SINK. mix_movie() drains this ring from
 * audio_callback -- but audio_callback belongs to g_dev, the OpenSL software
 * mixer, and a Unity title never opens g_dev (see the note above
 * audio_fmod_open). So in this port the ring is drained by the FMOD pump
 * instead, via opensles_movie_mix_s16, and opensles_movie_begin returns the
 * FMOD device's rate rather than opening a second SDL audio device.
 *
 * opensles_movie_begin returns 0 when there is no sink at all -- which is a
 * normal state, not an error: the FMOD pump only opens its device after its
 * warm-up frames, so a clip that starts before then simply plays silent. */
...
/* Mix up to `frames` stereo frames of movie audio into an S16 interleaved
 * buffer that already holds the engine's PCM, saturating. Returns frames mixed.
 * Called from the FMOD pump thread: MUST NOT LOG and MUST NOT do file IO. */
...
int      audio_fmod_rate(void);                        // 0 until the device is open
...
/* Audio pipeline counters, formatted for the main thread to log. Safe to call
 * from any thread; the counters are plain and only ever increase. */
