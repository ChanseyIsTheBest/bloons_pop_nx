...
#include "config.h"
#include "bp_config.h"
#include <switch.h>      /* fsdevCommitDevice, Result: the prefs store commits the SD card */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
...
static int   g_kv_loaded_n = 0;    /* entries present at load: the empty-write guard */
static int   g_kv_wipe_ok  = 0;    /* set only by an explicit PlayerPrefs.DeleteAll */
static int   g_kv_incomplete = 0;  /* load or alloc lost data: do not overwrite the file */

static char *prefs_file(char *buf,size_t n){ snprintf(buf,n,"%s/prefs.kv",g_root); return buf; }

/* Every allocation here is checked, because the failure mode is not "this one
 * pref is lost" -- it is a store that flush will then write over a good save.
 * On failure the previous value is kept and the store is marked unsafe to
 * write, which is strictly better than persisting a hole. */
static void kv_set(char type,const char*key,const char*val){
  for (int i=0;i<g_kv_n;i++) if(!strcmp(g_kv[i].key,key)){
    char *nv = strdup(val);
    if (!nv) { g_kv_incomplete = 1;
               debugPrintf("[prefs] strdup failed updating '%s' -- keeping old value\n", key);
               return; }
    g_kv[i].type=type; free(g_kv[i].val); g_kv[i].val=nv; g_kv_dirty=1; return; }

  if (g_kv_n==g_kv_cap){
    int cap = g_kv_cap ? g_kv_cap*2 : 32;
    KV *grown = realloc(g_kv, (size_t)cap*sizeof(KV));
    if (!grown) { g_kv_incomplete = 1;
                  debugPrintf("[prefs] realloc(%d) failed -- '%s' dropped\n", cap, key);
                  return; }
    g_kv = grown; g_kv_cap = cap;
  }
  char *nk = strdup(key), *nv = strdup(val);
  if (!nk || !nv) { free(nk); free(nv); g_kv_incomplete = 1;
                    debugPrintf("[prefs] strdup failed adding '%s'\n", key); return; }
  g_kv[g_kv_n].type=type; g_kv[g_kv_n].key=nk; g_kv[g_kv_n].val=nv;
  g_kv_n++; g_kv_dirty=1;
}
...
/* Read one prefs file into the store. Returns entries read, or -1 if absent. */
static int prefs_read_file(const char *path) {
  /* Reads the WHOLE file, then splits it. The previous version parsed with
   * fgets into char[2048], while the writer accepts values up to VALMAX
   * (65536) and esc() can double a value's length. Anything longer than the
   * buffer was silently TRUNCATED and its tail re-parsed as a fresh line --
   * so one long value both shortened its own entry and could invent a bogus
   * key from the remainder, and the next flush wrote that back permanently.
   * Silent, progressive corruption with no crash to point at it.
   *
   * The file is a few KB; reading it whole removes the ceiling entirely rather
  ...
  if (!buf) { fclose(f); debugPrintf("[prefs] malloc(%ld) failed reading %s\n", sz, path);
              return -1; }
  ...
  while (p < end) {
    ...
    char *line = p;
    p = nl ? nl + 1 : end;
    while (len && line[len-1] == '\r') len--;      /* tolerate CRLF */
    line[len] = 0;
    if (!len) continue;

    /* Strict shape: T \t key \t value. Anything else is counted, not guessed
     * at -- a malformed line used to be skipped silently, which hid exactly
     * the corruption above. */
    if (len < 4 || line[1] != '\t') { bad++; continue; }
    char type = line[0];
    char *k = line + 2;
    char *t1 = strchr(k, '\t');
    if (!t1) { bad++; continue; }
    *t1 = 0;
    char *v = t1 + 1;
    if (!*k) { bad++; continue; }                  /* empty key is never valid */
  ...
  if (bad) {
    g_kv_incomplete = 1;
    debugPrintf("[prefs] %s: %d malformed line(s) -- the store is INCOMPLETE and "
                "will not be written back over the file\n", path, bad);
  }
  return g_kv_n - n0;
}
...
static void prefs_load(void){
   ...
   *
   * prefs_flush keeps the previous good save as .bak and swaps atomically, so
   * if the main file is missing or empty the backup is the last known-good
   * state. Reaching for it costs one fopen and is the difference between
   * losing a session and losing everything -- which this port has now done to
   * a real 43-entry save once. */
  if (n <= 0) {
    int b = prefs_read_file(bak);
    if (b > 0) {
      debugPrintf("[prefs] %s was %s -- recovered %d entries from %s\n",
                  p, n < 0 ? "missing" : "empty", b, bak);
      n = b;
      g_kv_dirty = 1;          /* write it back out as the primary */
    } else if (n < 0) {
      debugPrintf("[prefs] load: no save file at %s (errno=%d)\n", p, errno);
    }
  }

  if (n > 0) g_kv_dirty = g_kv_dirty ? 1 : 0;
  g_kv_loaded_n = g_kv_n;      /* what the empty-write guard compares against */

  /* Report BYTES, not just a count -- N entries totalling almost nothing is a
   * save that loaded structurally and is empty in substance, and a bare count
   * cannot tell those apart. */
  {
    size_t bytes = 0; int empty = 0;
    for (int i = 0; i < g_kv_n; i++) {
      size_t k = g_kv[i].val ? strlen(g_kv[i].val) : 0;
      bytes += k;
      if (!k) empty++;
    }
    debugPrintf("[prefs] load: %d entries (%u value bytes, %d empty) from %s\n",
                g_kv_n, (unsigned)bytes, empty, p);
    for (int i = 0; i < g_kv_n && i < 24; i++)
      debugPrintf("[prefs]   %c %-40s %u bytes\n", g_kv[i].type,
                  g_kv[i].key ? g_kv[i].key : "?",
                  (unsigned)(g_kv[i].val ? strlen(g_kv[i].val) : 0));
  }
}
static void prefs_flush(void){
  /* Silent when clean. PlayerPrefs.Save() is called by CustomPlayerPrefs on
   * every set with isSaveImmediately, so this runs constantly; logging here
   * floods the log and buries everything else. A no-op flush is not news. */
  if(!g_kv_dirty) return;

  /* GUARD: never let an empty store overwrite a save that had content.
   *
   * A run wiped a 43-entry save. prefs_flush writes the WHOLE store, so
   * anything that empties it -- a failed load, a half-initialised store, a
   * DeleteAll the game did not mean -- destroys the file. ticket_to_earth_nx
   * carries this guard and sec.51 declined to adopt it because their version
   * was scoped to specific key names. The principle was not theirs to keep:
   * "an empty snapshot must not clobber a good one" needs no key list.
   *
   * DeleteAll sets g_kv_wipe_ok, so a deliberate reset still works. */
  /* A large shrink is not blocked -- DeleteKey exists and a game may legitimately
   * prune -- but it is worth seeing in the log, because "the save got smaller"
   * is otherwise invisible until the player notices progress missing. */
  if (g_kv_loaded_n > 4 && g_kv_n < g_kv_loaded_n / 2 && g_kv_n > 0)
    debugPrintf("[prefs] NOTE: store shrank %d -> %d entries since load\n",
                g_kv_loaded_n, g_kv_n);

  if (g_kv_n == 0 && g_kv_loaded_n > 0 && !g_kv_wipe_ok) {
    debugPrintf("[prefs] REFUSED to write an empty store over a %d-entry save "
                "-- keeping the file on disk\n", g_kv_loaded_n);
    g_kv_dirty = 0;
    return;
  }

  /* The store is known to be missing data -- a malformed line on load, or an
   * allocation that failed. Writing it back would make a recoverable file
   * unrecoverable, since flush replaces the whole thing. Keep the file, keep a
   * copy of it under .corrupt so the player has something to send, and stop
   * writing for the rest of the session. */
  if (g_kv_incomplete) {
    static int said;
    if (!said) {
      char cp[288], src[256];
      prefs_file(src, sizeof src);
      snprintf(cp, sizeof cp, "%s.corrupt", src);
      FILE *in = fopen(src, "rb"), *out = in ? fopen(cp, "wb") : NULL;
      if (in && out) {
      ...
      if (out) fclose(out);
      if (in) fclose(in);
      if (in && out) fsdevCommitDevice("sdmc");
      debugPrintf("[prefs] REFUSED to write an incomplete store. Original kept "
                  "at %s%s\n", cp, (in && out) ? "" : " (copy FAILED)");
      said = 1;
    }
    g_kv_dirty = 0;
    return;
   ...
   *
   * fopen(path,"wb") truncates BEFORE anything is written, so the old code had
   * a window in which prefs.kv was empty on disk. A crash in that window --
   * and this port had an intermittent boot crash for several builds -- loses
   * the save outright. Rename is atomic, so a crash now leaves either the old
   * complete file or the new one, never a truncated one. */
  char p[256], tmp[288], bak[288];
  prefs_file(p, sizeof p);
  ...
  fclose(f);
  if (werr) {
    debugPrintf("[prefs] flush FAILED: write error on %s -- old save kept\n", tmp);
    remove(tmp);
    return;
  }

  /* Commit the temp file before the rename, so the data is on the card before
   * anything points at it. */
  fsdevCommitDevice("sdmc");

  remove(bak);
  rename(p, bak);              /* previous good save -> .bak (may not exist) */
  if (rename(tmp, p) != 0) {
    debugPrintf("[prefs] flush FAILED: rename(%s -> %s) errno=%d\n", tmp, p, errno);
    rename(bak, p);            /* put the old one back */
    return;
  }

  g_kv_dirty = 0; g_kv_wipe_ok = 0;
  { extern int nx_sd_dirty; nx_sd_dirty = 0; }
  Result rc = fsdevCommitDevice("sdmc");
  if (R_FAILED(rc))
    debugPrintf("[prefs] flush: wrote %d entries but COMMIT FAILED rc=0x%x\n", g_kv_n, rc);
  else
    debugPrintf("[prefs] flush: %d entries -> %s (atomic, committed)\n", g_kv_n, p);
}

/* PlayerPrefs write-through -- THIS IS THE LIVE SAVE PATH FOR BOUNCEMASTERS.
 *
 * badpiggies_nx fed this store from il2cpp PlayerPrefs hooks, because Unity
 * 2020.3 implemented Android PlayerPrefs natively and its writes never reached
 * disk on Switch. Those hooks are gone (they were Bad Piggies RVAs) and are not
 * needed here: Unity 6 goes back through Java SharedPreferences, which is the
 * path implemented right here.
 *
 * Verified against THIS game's libunity.so, which carries all three markers:
 * "getSharedPreferences", "android/content/SharedPreferences" and the
 * ".v2.playerprefs" store name. Bouncemasters is Unity 6000.0.58f2, the same
 * generation cloverpit_nx verified this on.
 *
 * That matters more than usual for this title. Bouncemasters keeps progression,
 * currency and offline-earning timestamps in PlayerPrefs, and its cloud-save
 * route is Epic Online Services -- which bp_sdk_stubs.c deliberately fails. This
 * is therefore the ONLY place player progress persists. If saves do not survive
 * a relaunch, look here first.
 */
...
/* --------------------------------------------------------------------------
 * Direct KV access for bp_prefs.c.
 *
 * The game's saves reach UnityEngine.PlayerPrefs (see bp_prefs.c for the chain)
 * and the engine's own Android implementation of it never called into this
 * file: zero [prefs] put lines across a whole session with currency being
 * collected. Rather than keep guessing why the native side stays silent,
 * bp_prefs.c hooks PlayerPrefs itself and stores here, which is the same store
 * that already persists to prefs.kv and commits.
 * ------------------------------------------------------------------------ */
void bp_prefs_set(char type, const char *k, const char *v) {
  if (!k || !k[0]) return;                 /* empty key: see putString above */
  kv_set(type, k, v ? v : "");
}
const char *bp_prefs_get(const char *k) {
  KV *kv = k ? kv_get(k) : NULL;
  return kv ? kv->val : NULL;
}
int  bp_prefs_has(const char *k)    { return k && kv_get(k) ? 1 : 0; }
void bp_prefs_delete(const char *k) {
  if (!k) return;
  for (int i = 0; i < g_kv_n; i++) if (!strcmp(g_kv[i].key, k)) {
    free(g_kv[i].key); free(g_kv[i].val);
    g_kv[i] = g_kv[--g_kv_n];              /* order is irrelevant here */
    g_kv_dirty = 1; return;
  }
}
void bp_prefs_delete_all(void) {
  /* Only a real PlayerPrefs.DeleteAll() reaches here, so this is the one case
   * where writing an empty store is intended. Everything else that empties the
   * store is a bug, and prefs_flush refuses to persist it. */
  debugPrintf("[prefs] DeleteAll: wiping %d entries (explicit)\n", g_kv_n);
  for (int i = 0; i < g_kv_n; i++) { free(g_kv[i].key); free(g_kv[i].val); }
  g_kv_n = 0; g_kv_dirty = 1; g_kv_wipe_ok = 1;
}
void bp_prefs_commit(void) { prefs_flush(); }
...
void *unity_dispatch_object(void *recv, const void *id_, va_list va){ const struct FakeID *id = id_;
  ...
  if (has(cls,"content/Context") || has(cls,"app/Activity")){   /* Activity IS-A Context */
    ...
    if (has(m,"getPackageName")) return jni_make_string(CS_PACKAGE);   /* config.h: this game, not the layer's donor */
...
uint64_t unity_dispatch_int(void *recv, const void *id_, va_list va){ const struct FakeID *id = id_;
  ...
  if (has(cls,"view/Display")||has(cls,"DisplayMetrics")||has(cls,"DisplayManager")){
    ...
    if (has(m,"orientation")) return 1;   /* Configuration.ORIENTATION_PORTRAIT: Bloons Pop is portrait-only */
...
void unity_jni_init(const char *data_root){
  ...
  prefs_load();
  bp_config_sync_prefs();   /* config.txt resolution -> Unity's saved screen size */
