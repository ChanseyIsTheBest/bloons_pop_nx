...
static void do_touch(void) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    float x, y;
    (void)sx; (void)sy;
    bp_tate_map_pointer((float)ts.touches[i].x, (float)ts.touches[i].y,
...
static void do_gyro(void) {
  ...
  if (fabsf(dx) > 0.05f || fabsf(dy) > 0.05f) {
    { float _rx, _ry; bp_tate_map_stick(dx, dy, &_rx, &_ry); s_cx += _rx;
    s_cy += _ry; }
    clamp_cursor();
  }
}
...
static int do_mouse(void) {
  ...
  if (dx || dy) {
    { float _rx, _ry; bp_tate_map_stick((float)dx * s_mouse_sens, (float)dy * s_mouse_sens, &_rx, &_ry); s_cx += _rx;
    s_cy += _ry; }
    clamp_cursor();
...
void nxp_update(void) {
  ...
  if (s_visible > 0) {
    ...
    if (ls.x || ls.y) {
      ...
      float rdx, rdy;
      bp_tate_map_stick((ls.x / 32767.0f) * s_stick_speed,
