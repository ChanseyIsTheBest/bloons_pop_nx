...
/* newlib ships no <sys/uio.h> -- its generic libc/include/sys has no uio.h at
 * all (only the spu, rtems and cygwin ports do), so anything taking an iovec
 * has to declare the layout itself. It is ABI-fixed on AArch64 LP64 and is just
 * {pointer, length}, which is what Bionic passes.
 *
 * Declared here rather than privately in each user, so the readv/writev shim in
 * libc_shim.c and the writev import in bp_missing_syms.c cannot drift apart --
 * two files each with their own "obviously it is just a pointer and a length"
 * struct is how a field-ordering mistake survives review. */
...
/* Force the debug log's SD buffer out now. debugPrintf otherwise flushes on a
 * 2 s timer, because flushing per line puts an fsFileWrite on the engine's main
 * thread. Call it on the way out so the last lines before an exit or a crash
 * actually reach the card. Defined in libc_shim.c. */
void nx_sd_flush(void);

void gc_paused_live_reset(void);   /* crash handler: after diag_resume_all_gc_paused() */
int bp_path_is_open(const char *path);   /* an fd of ours is open on path (Horizon locks open files) */
