...
/* Tripwire -- a flag whose header is missing evaluates to 0 and would silently
 * open the device at 0 Hz. See tools/check_flags.sh. */
#ifndef BP_AUDIO_DEVICE_RATE
#error "config.h not included -- BP_AUDIO_DEVICE_RATE would compile out silently"
#endif
#include "util.h"
...
typedef struct {
  const void *data;   // -> own; never FMOD's memory (see bq_Enqueue)
  SLuint32 size;
...
/* Rate the movie producer must resample to: the FMOD device's, or g_dev's when
 * BP_VIDEO_OWN_AUDIO_DEVICE is on. 0 until opensles_movie_begin resolves one. */
static int g_movie_dev_rate = 0;
/* EXACTLY ONE drain may own the ring. mix_movie() runs from audio_callback on
 * g_dev; opensles_movie_mix_s16() runs from the fmodProcess pump on g_fmod_dev.
 * If both ran, each would consume half the frames and the movie audio would
 * come out as noise rather than as sound. Resolved once, in
 * opensles_movie_begin, from whichever device is actually open. */
#define MOVIE_SINK_NONE      0
#define MOVIE_SINK_MIXER     1
#define MOVIE_SINK_FMOD_PUMP 2
static int g_movie_sink = MOVIE_SINK_NONE;
...
/* --- diagnostic counters --------------------------------------------------
 * The audio callback and FMOD's mixer thread must not do file I/O: logging from
 * them is what stopped this console booting two builds ago. So they only bump
 * plain counters here, and bp_audio_stats() formats them for the MAIN thread to
 * print. Enough to separate the two candidate causes without another guess:
 *
 *   consumed > enqueued   -> the mixer is replaying blocks (duplication)
 *   dry high              -> the queue empties mid-callback (underrun)
 *   short high            -> callbacks ending early, i.e. partial silence
 *
 * "short" counts callbacks that ran out of data part-way and left the rest of
 * the block silent -- the exact shape of a click. */
static volatile uint32_t g_st_enq, g_st_consumed, g_st_dry, g_st_short, g_st_cb;

void bp_audio_stats(char *out, size_t cap) {
  snprintf(out, cap,
           "enq=%u consumed=%u cb=%u dry=%u short=%u",
...
static void mix_movie(int32_t *acc, int frames) {
  if (g_movie_sink != MOVIE_SINK_MIXER)
    return;
  if (!g_movie_lock)
    return;
...
static void ensure_device(int rate) {
  // Historically forced to 48000 and resampled, because players arrived at mixed
  // rates (22050/44100) and letting the first one pin the device was worse. With
  // FMOD as the sole producer there is one rate, so request BP_AUDIO_DEVICE_RATE
  // and let mix_player's resampler handle whatever we are actually granted.
  (void)rate;
  if (!g_reg_lock)
  ...
  SDL_zero(want);
  want.freq = BP_AUDIO_DEVICE_RATE;
  want.format = AUDIO_S16SYS;
  want.channels = 2;
  want.samples = BP_AUDIO_CALLBACK_FRAMES;
  want.callback = audio_callback;
  // ALLOW_FREQUENCY_CHANGE: if the backend cannot do the requested rate it
  // substitutes its own rather than failing the open, and g_dev_rate follows so
  // the resampler stays correct. Degrades to the old behaviour, never to silence.
  ...
  // Init path only -- deliberately NOT in audio_callback or bq_Enqueue. The
  // previous round's instrumentation logged from those and the console stopped
  // booting; this line runs once, before any audio thread exists.
  debugPrintf("[fmod] SDL mixer device: asked %d Hz, got %d Hz, %d ch%s\n",
              BP_AUDIO_DEVICE_RATE, have.freq, have.channels,
              have.freq == BP_AUDIO_DEVICE_RATE ? " (no resampling)"
...
int opensles_movie_begin(int requested_rate) {
  ...
  /* WHICH DEVICE ACTUALLY DRAINS THE RING.
   *
   * CORRECTED FROM A HARDWARE LOG. An earlier revision of this function
   * refused to touch g_dev, reasoning from the comment above audio_fmod_open
   * that "a Unity title never opens g_dev" and that calling ensure_device()
   * here would therefore open a SECOND SDL audio device. The log says
   * otherwise, and the comment was stale:
   *
   *     [fmod] slCreateEngine() -- FMOD OpenSL output init reached
   *     [fmod] OpenSL CreateAudioPlayer: 24000 Hz, 2 ch (FMOD output player)
   *     [fmod] SDL mixer device: asked 24000 Hz, got 24000 Hz, 2 ch
   *     [fmod] OpenSL buffer-queue callback firing -> FMOD mixer is producing PCM
   *
   * Unity 6's FMOD Android backend reaches OpenSL ES through slCreateEngine,
   * which this shim serves, so g_dev IS the port's one and only audio device
   * and mix_movie() -- already called from audio_callback every block -- was
   * correctly wired all along. The fmodProcess pump in jni_fake.c never runs
   * in this build at all: there is no "[fmod] output device opened" line and
   * no "[fmod] pump start" line anywhere in the log.
   *
   * So ask for g_dev first. ensure_device() is idempotent and by the time a
   * clip plays it has long since been opened by the engine's own audio init.
   * The pump device stays as a fallback for a configuration where it, rather
   * than OpenSL, is the live path. */
  if (g_movie_sink == MOVIE_SINK_NONE) {
    ensure_device(requested_rate > 0 ? requested_rate : 44100);
    if (g_dev) {
      g_movie_sink = MOVIE_SINK_MIXER;      /* mix_movie(), audio_callback */
      g_movie_dev_rate = g_dev_rate;
    } else {
      int fmod_rate = audio_fmod_rate();
      if (fmod_rate <= 0)
        return 0;                            /* no sink: caller plays silent */
      g_movie_sink = MOVIE_SINK_FMOD_PUMP;   /* opensles_movie_mix_s16() */
      g_movie_dev_rate = fmod_rate;
    }
  }
  ...
  SDL_UnlockMutex(g_movie_lock);
  return g_movie_dev_rate;
}

/* The FMOD pump's drain of the movie ring. Deliberately a near-copy of
 * mix_movie() rather than a refactor of it: mix_movie accumulates into the
 * int32 scratch that the OpenSL software mixer builds, this one saturates
 * straight into the S16 block FMOD has already filled, and only one of the two
 * is ever live in a given port. Keeping them separate means touching the video
 * path cannot perturb the OpenSL path.
 *
 * NO LOGGING AND NO FILE IO IN HERE -- audio pump thread. */
int opensles_movie_mix_s16(int16_t *dst, int frames) {
  if (g_movie_sink != MOVIE_SINK_FMOD_PUMP)
    return 0;
  if (!g_movie_lock || !dst || frames <= 0)
    return 0;
  ...
  for (int i = 0; i < n; i++) {
    const int idx = (g_movie_head + i) % MOVIE_RING_FRAMES;
    for (int c = 0; c < 2; c++) {
      int32_t v = (int32_t)dst[i * 2 + c] + (int32_t)g_movie_pcm[idx * 2 + c];
      if (v > 32767)       v = 32767;
      else if (v < -32768) v = -32768;
      dst[i * 2 + c] = (int16_t)v;
    }
...
int opensles_movie_queue(const int16_t *pcm, int frames) {
  ...
  while (done < frames) {
    ...
    if (done < frames) {
      /* BOUNDED. This used to wait indefinitely for the mixer to make room,
       * which is a second way for a full ring to stall the caller -- and the
       * caller is the movie decode thread, which also drives video. Waiting
       * here for longer than a video frame trades a few milliseconds of audio
       * for a visible stall, which is the wrong way round. Give the mixer a
       * short window and then drop the remainder. */
      if (++waited > 20)                /* ~40 ms */
        break;
      SDL_Delay(2);
    }
...
/* 0 until the pump has opened the device. opensles_movie_begin uses this to
 * decide whether movie audio has a sink at all. */
int audio_fmod_rate(void) {
  return g_fmod_dev ? g_fmod_dev_rate : 0;
}
...
/* COPY, do not alias.
 *
 * This queue used to store FMOD's pointer and the mixer read through it later,
 * on the SDL audio thread. That is a data race with FMOD's own ring: the shim
 * has no idea when FMOD reuses or rewrites a block it has already handed over.
 * The audible result was stale blocks replayed as repeated sound effects and
 * torn block boundaries as constant clicking -- at EVERY queue depth, which is
 * what ruled out the buffer count as the cause. Enqueue means "this data is
 * ready now", so take it at its word and take a copy now.
 *
 * The allocation settles after the first block or two: FMOD's period is
 * constant, so realloc fires once per slot and never again. */
...
/* Release the per-slot copies bq_Enqueue allocated. Both player free paths must
 * call this: the engine never Destroys finished SEs, so the reclaim path in
 * eng_CreateAudioPlayer recycles players continuously and a miss here leaks a
 * block per slot, forever, on a console already reporting 3 MB free. */
