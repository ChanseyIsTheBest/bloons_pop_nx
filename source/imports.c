...
int bp_path_is_open(const char *path);   /* libc_shim.c */
#include <stdlib.h>
#include <stdio.h>
#include <stdint.h>
#include <stdarg.h>
...
/* Defined in bp_missing_syms.c -- see the table rows below. */
extern int     __android_log_buf_write(int bufID, int prio, const char *tag, const char *text);
extern int     AImage_getWidth(void *image, int32_t *width);
extern double  bp_nextafter(double x, double y);
extern ssize_t bp_writev(int fd, const struct nx_iovec *iov, int iovcnt);

/* ---------------------------------------------------------------------------
 * clock_gettime: map the Linux clock ids libnx refuses onto ones it accepts.
 *
 * libnx's __syscall_clock_gettime is strict:
 *
 *     if (clock_id != CLOCK_MONOTONIC && clock_id != CLOCK_REALTIME) {
 *         errno = EINVAL; return -1;
 *     }
 *
 * and on failure it leaves the caller's timespec UNTOUCHED -- so a caller that
 * does not check the return value silently reads whatever was already in that
 * stack slot.
 *
 * Unity's baselib does exactly that. Baselib_Timer_GetTimeSinceStartupInSeconds
 * calls clock_gettime twice:
 *
 *     mov w0, #1    ; CLOCK_MONOTONIC  -> succeeds, fills [sp]
 *     ...
 *     mov w0, #7    ; CLOCK_BOOTTIME   -> EINVAL, [sp] unchanged
 *     ldp d1, d2, [sp]                 ; re-reads the stale values
 *
 * CLOCK_BOOTTIME is 7. It is a perfectly ordinary clock on Android -- monotonic
 * but also counting time spent suspended -- and Unity reaches for it because on
 * a phone that distinction matters. Here it just fails.
 *
 * WHY THAT STOPPED THE BOOT
 *
 * GetTimeSinceStartup() is Baselib_Timer_GetTimeSinceStartupInSeconds() minus a
 * baseline, and Unity's SPLASH SCREEN advances on it:
 *
 *     IsSplashScreenFinished():
 *         if (state == 3) return true;
 *         elapsed = GetTimeSinceStartup() - splash->startTime;
 *         UpdateTimer(elapsed);          // needs elapsed > minimum duration
 *         return state == 3;
 *
 * and UnityPlayerLoop() gates the ENTIRE game loop on the answer:
 *
 *     +0x258  bl   IsSplashScreenFinished()
 *     +0x25c  tbz  w0, #0, +0x348        ; not finished -> jump past PlayerLoop
 *     ...
 *     +0x308  bl   PlayerLoop()          ; <- SKIPPED
 *     +0x348  ... DrawSplashScreen(bool) ; <- drawn instead, then Thread::Sleep
 *
 * So while the splash is unfinished the engine draws it and runs no PlayerLoop
 * at all. No Update stage means UniTask's Update-timing runner never ticks, and
 * GameLoadingService.LoadGame -- which awaits
 * UniTask.Delay(1, DeltaTime, PlayerLoopTiming.Update) between step groups --
 * never resumes. ApplicationLoadedSignal never fires, the game never leaves the
 * splash, and every thread looks perfectly healthy the whole time, because it
 * is. That is precisely the observed behaviour.
 *
 * Mapping is safe: on this console nothing suspends mid-session in a way that
 * would make BOOTTIME and MONOTONIC diverge meaningfully, and the alternative
 * is a call that fails and leaves the caller reading uninitialised stack.
 * ------------------------------------------------------------------------- */
int clock_gettime_fake(clockid_t clk, struct timespec *tp) {
  clockid_t use = clk;
  switch ((int)clk) {
    ...
    case 9:  /* CLOCK_BOOTTIME_ALARM   */
      use = CLOCK_MONOTONIC; break;
    case 5:  /* CLOCK_REALTIME_COARSE  */
    case 8:  /* CLOCK_REALTIME_ALARM   */
      use = CLOCK_REALTIME;  break;
    default: break;
  }
  int r = clock_gettime(use, tp);
  if (r != 0 && tp) {
    /* Never leave the timespec untouched on failure -- that is the trap that
     * made this invisible. A zeroed value is wrong but obvious; stale stack is
...
int pthread_create_fake(pthread_t *thread, const void *bionic_attr, void *entry, void *arg) {
  ...
  /* Allocate the thread stack OURSELVES, through __real_memalign.
   *
   * Letting newlib allocate it internally routes a 2 MB page-aligned request
   * through __wrap_memalign and into the GPU arena (bloonspop_gpuarena.c). libnx
   * needs a genuine stack allocation, so the new thread never came up and
   * PlatformThread::Create waited on its "ready" condvar forever -- observed as
   * UnityMain parked in pthread_cond_wait_fake inside nativeRender at frame 0,
   * with every worker idle.
   *
   * Going through __real_memalign keeps stacks in newlib where they belong and
   * leaves the arena for graphics buffers only, which is the one thing it is
   * supposed to hold. */
  extern void *__real_memalign(size_t, size_t);
  extern void  __real_free(void *);
  pthread_attr_t attr; pthread_attr_init(&attr);
  void *stk = __real_memalign(0x1000, stack);
  if (stk) pthread_attr_setstack(&attr, stk, stack);
  else     pthread_attr_setstacksize(&attr, stack);
  const int r = pthread_create(thread, &attr, thread_trampoline, ts);
  pthread_attr_destroy(&attr);
  if (r != 0 && stk) __real_free(stk);
...
static int access_impl_base(const char *path, int mode) {
  (void)mode; struct stat st; return stat(path, &st) == 0 ? 0 : -1;
}

/* Same Horizon rule as stat(): a file we hold open for write cannot be probed. */
static int access_impl(const char *path, int mode) {
  int r = access_impl_base(path, mode);
  if (r != 0 && path && bp_path_is_open(path)) return 0;
  return r;
}
...
static EGLBoolean egl_QuerySurface_log(EGLDisplay d, EGLSurface s, EGLint attr, EGLint *val) {
  ...
  if (val) {
    if (attr == 0x3057 /*EGL_WIDTH*/  && (*val <= 0 || bp_tate_enabled())) { *val = screen_width;  r = EGL_TRUE; }
    if (attr == 0x3056 /*EGL_HEIGHT*/ && (*val <= 0 || bp_tate_enabled())) { *val = screen_height; r = EGL_TRUE; }
  }
  return r;
}
...
static EGLBoolean egl_SwapBuffers_log(EGLDisplay d, EGLSurface s) {
  ...
  bp_tate_bind_overlay();   /* cursor lands in the portrait FBO, rotates with it */
  nxp_draw();   /* on-screen cursor overlay; saves/restores all GL state */
#endif
  /* Splash video overlay. Returns immediately unless a clip is decoding, and
   * saves/restores all GL state exactly as nxp_draw does. This is the only
   * place in the port with a guaranteed-current GL context on the presenting
   * thread, which is why the frame is drawn here rather than from the loop. */
  bp_video_draw();
  /* Present accounting across the frame-26 boundary. Everything so far measured
   * nativeRender as a whole; this narrows it to the present itself -- if the
   * last line in the log is "swap N enter" with no "ok", the console died inside
   * eglSwapBuffers and the swapchain state logged beside it says why. */
  {
    static unsigned long swap_n;
    extern void nx_window_report(const char *);
    unsigned long n = ++swap_n;
    int verbose = (n <= 40) || (n % 300 == 0);
    if (verbose) debugPrintf("[gfx] swap %lu enter\n", n);
    bp_tate_swap_hook();   /* rotated blit of the portrait FBO onto the window */
    EGLBoolean r = eglSwapBuffers(d, s);
    if (verbose) {
      EGLint e = eglGetError();
      debugPrintf("[gfx] swap %lu %s (egl=0x%04x)\n", n, r ? "ok" : "FAILED", e);
      if (n >= 20 && n <= 32) nx_window_report("post-swap");
    }
    return r;
  }
}
...
DynLibFunction dynlib_functions[] = {
  ...
  /* --- Bouncemasters delta (bp_missing_syms.c). The complete set of imports
   * this game needs that the inherited tables did not already cover: 4 out of
   * 498. Re-run tools/symcheck.py after any game update. --- */
  ...
  { "close", (uintptr_t)&bpn_close }, { "read", (uintptr_t)&read_fake },
  ...
  { "poll", (uintptr_t)&bpn_poll }, { "select", (uintptr_t)&bpn_select },
  { "dup2", (uintptr_t)&dup2_stub }, { "fcntl", (uintptr_t)&bpn_fcntl },
  { "ioctl", (uintptr_t)&bpn_ioctl }, { "isatty", (uintptr_t)&isatty },
  ...
  // --- networking (offline) ---
  { "socket", (uintptr_t)&bpn_socket }, { "connect", (uintptr_t)&bpn_connect },
  { "socketpair", (uintptr_t)&bpn_socketpair },  /* was unresolved */
  { "environ", (uintptr_t)&fake_environ },        /* was unresolved (il2cpp env reads) */
  { "bind", (uintptr_t)&bpn_bind }, { "listen", (uintptr_t)&bpn_listen },
  { "accept", (uintptr_t)&bpn_accept }, { "send", (uintptr_t)&bpn_send },
  { "recv", (uintptr_t)&bpn_recv }, { "sendto", (uintptr_t)&bpn_sendto },
  { "recvfrom", (uintptr_t)&bpn_recvfrom }, { "shutdown", (uintptr_t)&bpn_shutdown },
  { "setsockopt", (uintptr_t)&bpn_setsockopt }, { "getsockopt", (uintptr_t)&bpn_getsockopt },
  { "getsockname", (uintptr_t)&bpn_getsockname }, { "getpeername", (uintptr_t)&bpn_getpeername },
  { "getaddrinfo", (uintptr_t)&bpn_getaddrinfo }, { "freeaddrinfo", (uintptr_t)&bpn_freeaddrinfo },
  { "getnameinfo", (uintptr_t)&bpn_getnameinfo }, { "gethostname", (uintptr_t)&gethostname_fake },
  ...
  { "inet_aton", (uintptr_t)&bpn_inet_aton }, { "inet_pton", (uintptr_t)&bpn_inet_pton },
  ...
  /* 2020.3 libunity links zlib directly (DT_NEEDED libz.so) */
  { "inflateInit_", (uintptr_t)&inflateInit_ }, { "zlibVersion", (uintptr_t)&zlibVersion },
  ...
  { "glBindBuffer", (uintptr_t)&glBindBuffer }, { "glBindFramebuffer", (uintptr_t)&bp_gl_BindFramebuffer_tate },
...
/* Unity 6 needs a media-NDK import surface the 2020.3/2022.3 tables never had:
 * libmediandk, AImageReader and AHardwareBuffer, plus a couple of strays. All
 * refusal stubs -- the engine's VideoPlayer/AVPro backend and the ARCore image
 * reader must fail cleanly rather than fault, since neither exists here.
 * See bp_imports_extra.c. */
extern DynLibFunction bp_imports_extra[];
extern size_t         bp_imports_extra_num;

static void build_combined(void) {
  if (g_combined) return;
  g_combined_n = (int)dynlib_numfunctions + unity_dynlib_numfunctions
               + (int)firebase_extra_numfunctions + (int)bp_imports_extra_num;
  ...
  memcpy(g_combined + off, bp_imports_extra,
         bp_imports_extra_num * sizeof(DynLibFunction));
}
...
/* bp_vsync.c locks the engine's own vsync mutex, so it must use the SAME pthread
 * shims libunity was relocated against. Look them up in the table itself. */
uintptr_t bp_import_lookup(const char *name) {
  for (size_t i = 0; i < sizeof(dynlib_functions) / sizeof(dynlib_functions[0]); i++)
    if (dynlib_functions[i].symbol && strcmp(dynlib_functions[i].symbol, name) == 0)
