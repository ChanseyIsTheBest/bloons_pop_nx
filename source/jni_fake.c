...
static int nx_tag_known(uint32_t t) {
  return t == TAG_OBJECT || t == TAG_STRING || t == TAG_OBJARR ||
         ...
         t == BITMAP_TAG ||
         t == 0x554a4831u /* UJ_TAG,  unity_jni.c   'UJH1' */ ||
         t == 0x55494531u /* UI_TAG,   unity_input.c 'UIE1' */ ||
         /* These three were MISSING and a tester's log caught it:
          *   [jni] tag-warn: unrecognised tag tag=0x554c5331 ('ULS1')
          * right after java/util/List.iterator. An unlisted tag makes
          * nx_tag_of() return 0 -- "not one of ours" -- so every dispatch and
          * every guard downstream takes its wrong branch for that object, for
          * the rest of the session, without anything failing at the point of
          * damage. They came in with the clonehero_nx unity_input.c (sec 12);
          * its tags were never added here. tools/tagcheck.py now cross-checks
          * the two lists so this cannot drift again. */
         t == 0x55494432u /* UID_TAG, unity_input.c 'UID2' */ ||
         t == 0x554c5331u /* ULS_TAG, unity_input.c 'ULS1' */ ||
         t == 0x554d5231u /* UMR_TAG, unity_input.c 'UMR1' */;
}
...
static int in_proxy_pool(const void *p);   /* defined with the pool, far below */
...
static const char *safe_class_name(void *p) {
   ...
   *
   * THREE different structs wear TAG_CLASS: FakeClass, FakeObject (tagged that
   * way by jni_make_object so free_ref ignores it) and FakeProxy (same trick in
   * proxy_make). The first two are interchangeable here BY LUCK and by design --
   * FakeClass::name and FakeObject::label are both a char array at offset 4, and
   * the dispatch at the bottom of this file reads an object's label through this
   * function on purpose.
   *
   * FakeProxy is NOT: at offset 4 it has `uint32_t pad; long long ptr`. Reading
   * that as a string returns raw pointer bytes, and since nothing inside those
   * twelve bytes is guaranteed to be zero, the read runs on into the next pool
   * entry. Exclude proxies explicitly -- by ADDRESS, the way proxy_ptr_of()
   * already does it, because the tag cannot tell them apart. */
  uintptr_t v = (uintptr_t)p;
  if (!p || (v & 3u) || v < 0x1000u || v >= NX_USER_AS_END) return "";
  if (*(volatile uint32_t *)p != TAG_CLASS) return "";
  if (in_proxy_pool(p)) return "";        /* not a name; would read past the struct */
...
// Off in release (BP_JNI_LEDGER 0) -- it costs a string compare per guess.
// ---------------------------------------------------------------------------
...
/* ---- third-party SDK classes (bloonspop_nx) --------------------------------------
 * Ads, attribution, stores and partner SDKs have no Java side on Switch. The
 * generic layer answers unknown calls with plausible fakes, which is right for
 * the engine's own android.* traffic but leads these SDKs deeper into paths that
 * assume a live Java SDK (the first hardware run spent its last seconds inside
 * Kongregate's bridge). Answer them honestly instead: null / false / 0, the
 * shape of "SDK not installed", which their C# layers already handle. */
static const char *const k_sdk_null_prefix[] = {
  "com/kongregate/", "com/ironsource/", "com/facebook/", "com/adjust/", "com/vungle/",
  "com/applovin/", "com/google/android/gms/ads", "com/google/ads/", "com/unity3d/ads/",
  "com/unity3d/services/", "com/appsflyer/", "com/chartboost/", "com/tapjoy/",
  "com/supersonic", "com/google/firebase/", "com/crashlytics/", "com/android/billingclient/",
  "com/google/android/play/", "com/onevcat/uniwebview", "com/unity/androidnotifications", NULL };
static int bp_sdk_class_is_null(const char *cls) {
#if BP_JNI_SDK_NULL
  if (!cls) return 0;
  for (int i = 0; k_sdk_null_prefix[i]; i++) {
    if (!strncmp(cls, k_sdk_null_prefix[i], strlen(k_sdk_null_prefix[i]))) {
      static const char *logged[32]; static int nlog;
      int seen = 0;
      for (int k = 0; k < nlog; k++) if (logged[k] == k_sdk_null_prefix[i]) seen = 1;
      if (!seen && nlog < 32) { logged[nlog++] = k_sdk_null_prefix[i];
        debugPrintf("[jni] SDK %s* -> inert (objects inert, values false/0; no Java SDK on Switch)\n", k_sdk_null_prefix[i]); }
      return 1;
    }
  }
#else
...
#define BP_SDK_NULL_RET(id, ...) do { if ((id) && bp_sdk_class_is_null((id)->cls)) return __VA_ARGS__; } while (0)

/* "SDK present but dormant": an object-returning SDK call hands back an inert
 * object of the SDK's own class, so the C# wrapper holds a NON-NULL handle and
 * every later call on it is answered by this same policy (false / 0 / inert).
 * Null is the wrong answer: Kongregate's AndroidWrapper binds
 * Func<string,object[],T> delegates to its Java object without a null check, and
 * IL2CPP invokes a null-target delegate with every argument shifted one
 * register -- the deterministic crash in both hardware runs (x23 == 0 at
 * Func.Invoke, Call<bool> reading Func.Invoke's own MethodInfo as its RGCTX). */
static void *bp_sdk_inert_object(const FakeID *id) {
  const char *r = strrchr(id->sig, ')');   /* sig is an embedded array */
  if (r && !strcmp(r + 1, "Ljava/lang/String;")) return jni_make_string("");
  if (r && r[1] == '[') return NULL;
  return jni_make_object(id->cls);
}
#define BP_SDK_OBJ_RET(id) do { if ((id) && bp_sdk_class_is_null((id)->cls)) return bp_sdk_inert_object(id); } while (0)
...
/* PER-THREAD, as in real JNI (bloonspop_nx). The inherited registry was ONE
 * process-wide stack of local refs and ONE stack of frame marks, so frames
 * pushed on different threads interleaved: thread A's PopLocalFrame popped
 * thread B's mark and freed B's refs while B was still using them. Unity's
 * curl jobs ask Java for the proxy (GetAndroidProxySettings ->
 * UnityPlayer.getNetworkProxySettings) on up to five job workers at once, each
 * inside its own Push/PopLocalFrame -- crash 01789149757 was a job worker
 * faulting in SetupCurlProxyServerBasedOnPlatformConfiguration. Now each thread
 * owns its table and frame stack. Everything stays under locals_lock, and a
 * registry of all tables keeps NewGlobalRef / DeleteLocalRef able to find a ref
 * made on another thread (the tolerance the shared stack had). Deleting leaves a
 * hole rather than swapping the last entry in, so frame marks stay exact. */
#define MAX_LOCALS 1048576          /* per thread */
...
typedef struct LocalTable {
  ...
  int ftop;
  struct LocalTable *next;          /* registry of every thread's table */
} LocalTable;
static __thread LocalTable *t_locals;
static LocalTable *g_tables;        /* guarded by locals_lock */
static Mutex locals_lock;

static LocalTable *my_locals(void) {             /* locals_lock held */
  LocalTable *t = t_locals;
...
static void lt_push(LocalTable *t, void *ref) {  /* locals_lock held */
  if (t->top == t->cap) {
    const int nc = t->cap ? t->cap * 2 : 256;
    if (nc > MAX_LOCALS) return;
    void **nr = realloc(t->refs, (size_t)nc * sizeof *nr);
    if (!nr) return;
    t->refs = nr;
    t->cap = nc;
  }
...
static int lt_remove(LocalTable *t, void *ref) { /* locals_lock held; 1 if found */
  for (int i = t->top - 1; i >= 0; i--) {
    if (t->refs[i] != ref) continue;
    t->refs[i] = NULL;                            /* hole: frame marks stay valid */
    const int floor = t->ftop ? t->frames[t->ftop - 1] : 0;
    while (t->top > floor && !t->refs[t->top - 1]) t->top--;
    return 1;
  }
  return 0;
}
/* Take ref out of whichever table holds it (the caller's first). locals_lock held. */
static int lt_forget(void *ref) {
  LocalTable *mine = my_locals();
  if (mine && lt_remove(mine, ref)) return 1;
  for (LocalTable *o = g_tables; o; o = o->next)
    if (o != mine && lt_remove(o, ref)) return 1;
  return 0;
}
 ...
 * ring and free it only once BP_JNI_QUARANTINE more have been retired. The
...
#if BP_JNI_QUARANTINE
static void   *g_retired[BP_JNI_QUARANTINE];
...
static void ref_retire(void *p) {
  ...
  g_retired_w = (g_retired_w + 1) % BP_JNI_QUARANTINE;
  g_retired_n++;
...
/* UnityPlayer.showSoftInput (2020.3): (String text, int type, boolean correction,
 * boolean multiline, boolean secure, boolean alert, String placeholder,
 * int characterLimit, boolean hideInput)V. Collect the arguments by position and
 * queue the keyboard (editbox.c shows it at the next frame boundary). Any other
 * keyboard-shaped call: first non-empty String, default keyboard. */
static void kbd_request(const FakeID *id, va_list va) {
  struct { char t; long long i; const char *s; } a[16];
  int n = 0;
  va_list ap; va_copy(ap, va);
  const char *p = strchr(id->sig, '(');
  if (p) for (p++; *p && *p != ')' && n < 16; p++) {
    a[n].i = 0; a[n].s = NULL;
    switch (*p) {
      case 'I': case 'Z': case 'B': case 'C': case 'S': a[n].t = 'I'; a[n].i = va_arg(ap, int); n++; break;
      case 'F': case 'D': (void)va_arg(ap, double); a[n].t = 'F'; n++; break;
      case 'J': a[n].t = 'J'; a[n].i = va_arg(ap, long long); n++; break;
      case '[':
        (void)va_arg(ap, void *); a[n].t = '['; n++;
        if (p[1] == 'L') { p++; while (*p && *p != ';') p++; } else if (p[1]) p++;
        break;
      case 'L': {
        const int is_str = !strncmp(p, "Ljava/lang/String;", 18);
        void *o = va_arg(ap, void *);
        while (*p && *p != ';') p++;
        a[n].t = 'L'; a[n].s = is_str ? obj_str(o) : NULL; n++;
        break;
      }
      default: break;
    }
  ...
  va_end(ap);
  if (n == 9 && a[0].t == 'L' && a[1].t == 'I' && a[6].t == 'L' && a[7].t == 'I') {
    editbox_request(a[0].s ? a[0].s : "", (int)a[1].i, (int)a[4].i, (int)a[3].i,
                    a[6].s ? a[6].s : "", (int)a[7].i);
    return;
  }
  const char *initial = "";
  for (int k = 0; k < n; k++) if (a[k].t == 'L' && a[k].s && a[k].s[0]) { initial = a[k].s; break; }
  editbox_request(initial, 0, 0, 0, "", 64);
}
...
static void *getproperty_value(const char *key) {
  ...
  if (which >= 0 && which <= 2 && !logged[which]) {
    ...
    debugPrintf("[jni] getProperty -> %s\n",
                which == 1 ? BP_JSTR(BP_AUDIO_DEVICE_RATE) : which == 2 ? BP_JSTR(BP_AUDIO_PERIOD_FRAMES) : "(empty)");
  }
  /* From config.h so the answer can never disagree with opensles.c or with the
   * FMOD period bp_patches.c forces (BP_AUDIO_PERIOD_FRAMES). */
  if (which == 1) return jni_make_string(BP_JSTR(BP_AUDIO_DEVICE_RATE));
  if (which == 2) return jni_make_string(BP_JSTR(BP_AUDIO_PERIOD_FRAMES));
...
static void *proxy_make(long long ptr) {
  ...
  p->tag = TAG_CLASS; p->pad = 0; p->ptr = ptr; g_last_proxy = p;
  /* TAG_CLASS so free_ref ignores it. `pad` is zeroed rather than left as
   * whatever the pool held: it sits where FakeClass keeps the first bytes of
   * its name, and anything reading this as a class deserves an empty string
   * rather than stale bytes. */
  return p;
}
/* Is this pointer one of our proxies? Range AND stride, so a pointer into the
 * middle of an entry is rejected. This is the pattern the tag cannot provide,
 * because proxies share TAG_CLASS with two other struct types. */
static int in_proxy_pool(const void *p) {
  uintptr_t a = (uintptr_t)p, lo = (uintptr_t)g_proxy_pool;
  uintptr_t hi = (uintptr_t)(g_proxy_pool + g_proxy_pool_n);
  return a >= lo && a < hi && ((a - lo) % sizeof(FakeProxy)) == 0;
}

static long long proxy_ptr_of(void *obj) {
  return in_proxy_pool(obj) ? ((FakeProxy *)obj)->ptr : 0;
}
...
static void *act_object(const FakeID *id, va_list va) {
  ...
  /* A launcher intent carries no data URI and no extras. Approximating them made
   * libunity treat the launch as a deep link (Application.deepLinkActivated with
   * a made-up URL) and hand the extras parser an object to read. */
  if (name_has(id->cls, "Intent") && (!strcmp(id->name, "getData") || !strcmp(id->name, "getDataString")))
    return NULL;
...
static juint act_int(const FakeID *id, va_list va) {
  ...
  if (is_editbox_show(id->name)) { editbox_mark_engine_driven(); kbd_request(id, va); return 1; }
...
static void act_void(const FakeID *id, va_list va) {
  ...
  if (is_editbox_show(id->name)) { editbox_mark_engine_driven(); kbd_request(id, va); return; }
...
static void *dispatch_object(void *recv, const FakeID *id, va_list va) {
  BP_SDK_OBJ_RET(id);   /* SDK policy at the shared core: instance, static (#define), nonvirtual, V and A all land here */
  jni_approx_arm();
  log_app_upcall(id);
...
static juint dispatch_int(void *recv, const FakeID *id, va_list va) {
  BP_SDK_NULL_RET(id, 0);   /* SDK policy at the shared core: instance, static (#define), nonvirtual, V and A all land here */
  jni_approx_arm();
  log_app_upcall(id);
  ...
  /* Keyboard calls BEFORE the ownership check: UnityPlayer belongs to unity_jni.c,
   * whose dispatch swallowed showSoftInput as a no-op (the tenth hardware run: no
   * [kbd] line at all). */
  if (is_editbox_show(id->name)) { editbox_mark_engine_driven(); kbd_request(id, va); return 1; }
  if (is_editbox_close(id->name)) { editbox_close(); return 1; }
...
static void dispatch_void(void *recv, const FakeID *id, va_list va) {
  BP_SDK_NULL_RET(id);   /* SDK policy at the shared core: instance, static (#define), nonvirtual, V and A all land here */
  log_app_upcall(id);
...
static void *j_NewGlobalRef(void *env, void *obj) {
  (void)env;
  mutexLock(&locals_lock);
  lt_forget(obj);                                /* no frame owns it any more */
  mutexUnlock(&locals_lock);
  return obj;
}
...
static juint j_PushLocalFrame(void *env, int cap) {
  ...
  mutexLock(&locals_lock);
  LocalTable *t = my_locals();
  if (t && t->ftop < MAX_FRAMES)
    t->frames[t->ftop++] = t->top;               /* this thread's frame only */
  mutexUnlock(&locals_lock);
  return 0;
}
static void *j_PopLocalFrame(void *env, void *result) {
  ...
  LocalTable *t = my_locals();
  if (t) {
    const int mark = t->ftop > 0 ? t->frames[--t->ftop] : 0;
    for (int i = mark; i < t->top; i++)          /* only refs THIS thread made */
      if (t->refs[i] && t->refs[i] != result)
        free_ref(t->refs[i]);
    t->top = mark;
    if (result) lt_push(t, result);
  }
  mutexUnlock(&locals_lock);
  return result;
}
...
#define APP_VERSION_NAME_FALLBACK CS_VERSION_NAME   /* config.h (was the donor's version) */
...
#define APP_VERSION_CODE CS_VERSION_CODE   /* config.h */
...
/* ---- UnityPlayer natives by name (bloonspop_nx) -------------------------------
 * bp_boot.c resolves every lifecycle entry point (initJni, nativeRender, ...) from
 * this capture, by NAME, rather than from an offsets header: a name survives a
 * game update, an offset does not. Names point into libunity's .rodata. */
#define BP_MAX_UNITY_NATIVES 64
static struct { const char *name; void *fn; } g_unity_natives[BP_MAX_UNITY_NATIVES];
static int g_unity_natives_n;
static void bp_note_unity_native(const char *name, void *fn) {
  if (!name || !fn) return;
  for (int i = 0; i < g_unity_natives_n; i++)
    if (!strcmp(g_unity_natives[i].name, name)) { g_unity_natives[i].fn = fn; return; }
  if (g_unity_natives_n < BP_MAX_UNITY_NATIVES) {
    g_unity_natives[g_unity_natives_n].name = name;
    g_unity_natives[g_unity_natives_n].fn = fn;
    g_unity_natives_n++;
  }
}
void *jni_lookup_unity_native(const char *name) {
  for (int i = 0; name && i < g_unity_natives_n; i++)
    if (!strcmp(g_unity_natives[i].name, name)) return g_unity_natives[i].fn;
  return NULL;
}
...
static juint j_RegisterNatives(void *env, void *cls, void *methods, int n) {
  ...
  if (m && name_has(cn, "unity3d/player/UnityPlayer")) {
    for (int i = 0; i < n; i++) {
      ...
      bp_note_unity_native(m[i].name, m[i].fn);
