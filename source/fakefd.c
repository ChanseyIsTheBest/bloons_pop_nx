...
#define MAX_FAKE_FDS 128   /* curl makes a socketpair per DNS lookup when loopback TCP is unavailable */
...
/* Readiness for poll()/select() over mixed fd sets (bp_net_shim.c). A read end
 * is readable when data is buffered OR every writer is gone (read returns 0,
 * EOF); a write end is writable while the ring has room. */
